...
def _parse_integer(
    value: str, field: str, row_number: int, issues: list[Issue]
) -> int | None:
    if _is_blank(value):
        issues.append(
            Issue(
    ...
    *,
    ...
    for row_number, row in enumerate(rows, start=1):
        ...
        for field in INTEGER_FIELDS:
            parsed_ints[field] = _parse_integer(
                row.get(field, ""), field, row_number, issues
            )

        for field in DECIMAL_FIELDS:
            parsed_decimals[field] = _parse_decimal(
                row.get(field, ""), field, row_number, issues
            )

        for field in DATE_FIELDS:
            parsed_dates[field] = _parse_date(
                row.get(field, ""), field, row_number, issues
            )
        ...
        if (
            ...
            and cantidad_bruta is not None
            and cantidad_bruta != cantidad
        ):
            issues.append(
                Issue(
                    severity="error",
        ...
        if (
            venta is not None
            and venta_bruta is not None
            and _round_money(venta_bruta) != _round_money(venta)
        ):
            issues.append(
                Issue(
                    severity="error",
        ...
        if cantidad is not None and precio is not None and venta is not None:
            if cantidad > 0 and _round_money(
        ...
        if (
            cantidad_bruta is not None
            and precio is not None
            and venta_bruta is not None
        ):
            if cantidad_bruta > 0 and _round_money(
                Decimal(cantidad_bruta) * precio
            ) != _round_money(venta_bruta):
                issues.append(
                    Issue(
                        severity="error",
        ...
        if (
            factor is not None
            and cantidad_neta is not None
            and volumen_neto is not None
        ):
        ...
        if (
            cantidad_neta is not None
            and cantidad_bruta is not None
            and cantidad_neta > cantidad_bruta
        ):
            issues.append(
                Issue(
                    severity="warning",
        ...
        if not _is_blank(group_key[0]) and not _is_blank(group_key[1]):
            group_state = groups.setdefault(
                group_key,
                {
                    ...
                    "metadata": {
                        field: row.get(field, "") for field in GROUP_METADATA_FIELDS
                    },
                    "rows": [],
                },
            )
...
def validate_csv(
    path: str | Path, *, strict_group_aggregates: bool = False
) -> list[Issue]:
    csv_path = Path(path)
